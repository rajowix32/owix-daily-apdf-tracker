...
def init_db():
    ...
    for col, defn in [
        ...
        try:
            c.execute(f"ALTER TABLE files ADD COLUMN {col} {defn}")
        except sqlite3.OperationalError:
            pass
    ...
    for idx_sql in [
        ...
        "CREATE INDEX IF NOT EXISTS idx_files_assigned ON files(assigned_to_id)",
        "CREATE INDEX IF NOT EXISTS idx_files_client ON files(client_id)",
        "CREATE INDEX IF NOT EXISTS idx_files_received ON files(date_received)",
        "CREATE INDEX IF NOT EXISTS idx_files_qc_picked ON files(qc_picked_by_id)",
        "CREATE INDEX IF NOT EXISTS idx_files_submission ON files(submission_date)",
    ]:
        c.execute(idx_sql)

    # Sessions table for browser-refresh-safe login
    ...
    c.execute("DELETE FROM sessions WHERE created_at < datetime('now', '-7 days')")
...
def get_active_clients():
    """Cached list of active clients — refreshes every 30 seconds."""
    return df_query("SELECT id, name, rate_per_page FROM clients WHERE status='Active' ORDER BY name")


@st.cache_data(ttl=30)
def get_active_team():
    """Cached list of active trainees/freelancers — refreshes every 30 seconds."""
    return df_query("SELECT id, full_name, role, team_function FROM users WHERE role IN ('trainee','freelancer') AND active=1 ORDER BY full_name")


@st.cache_data(ttl=30)
def get_all_batches():
    """Cached list of batches — refreshes every 30 seconds."""
    return df_query("SELECT DISTINCT batch FROM files WHERE batch IS NOT NULL AND batch != '' ORDER BY batch")
...
def _create_session_token(user_id: int) -> str:
    """Store a session token in DB and return it."""
    token = str(uuid.uuid4())
    conn = get_conn()
    conn.execute("INSERT OR REPLACE INTO sessions (token, user_id) VALUES (?,?)", (token, user_id))
    conn.commit()
    conn.close()
    return token


def _resolve_session_token(token: str):
    """Look up a URL token and return user info dict, or None if invalid/expired."""
    if not token:
        return None
    conn = get_conn()
    row = conn.execute("""
        SELECT u.id, u.full_name, u.role,
               COALESCE(u.team_function,'Production') AS team_function, u.active
...
def login_page():
    # On every load: check URL for an existing session token (survives browser refresh)
    try:
        token = st.query_params.get("token", "")
        if token:
            user = _resolve_session_token(token)
            if user:
                st.session_state.user_id    = user["id"]
                st.session_state.user_name  = user["full_name"]
                st.session_state.user_role  = user["role"]
                st.session_state.team_function = user["team_function"]
                st.session_state.session_token = token
                st.rerun()
                return
            else:
                # Token expired or user deactivated — clear the stale param
                try:
                    st.query_params.clear()
                except Exception:
                    pass
    except Exception:
        pass  # query_params not available in some environments
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            if submitted:
                if not username or not password:
                    ...
                    conn = get_conn()
                    row = conn.execute(
                        "SELECT id, full_name, role, COALESCE(team_function,'Production') AS team_function "
                        "FROM users WHERE username=? AND password_hash=? AND active=1",
...
def logout():
    # Delete session token from DB so the URL token is invalidated
...
def page_files_owner():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📋 All Files", "➕ Add New File", "📦 Bulk Add (Excel)", "🗑️ Bulk Delete"])
    ...
    with tab1:
        ...
        if files_df.empty:
            ...
            st.markdown("##### Edit or delete a single file")
    ...
    # ----- TAB 4: Bulk Delete -----
    with tab4:
        st.markdown("##### Bulk Delete Files")
        st.caption("Filter the files you want to delete, preview them, then confirm. This cannot be undone.")

        bd_col1, bd_col2, bd_col3 = st.columns(3)
        with bd_col1:
            bd_clients = df_query("SELECT id, name FROM clients ORDER BY name")
            bd_client = st.selectbox("Client", ["All"] + bd_clients["name"].tolist(), key="bd_client")
        with bd_col2:
            bd_batches_sql = "SELECT DISTINCT batch FROM files WHERE batch IS NOT NULL AND batch != '' ORDER BY batch"
            bd_batches = df_query(bd_batches_sql)["batch"].tolist()
            bd_batch = st.selectbox("Batch", ["All"] + bd_batches, key="bd_batch")
        with bd_col3:
            bd_status = st.selectbox("Status", ["All", "Pending", "In Progress", "Production Done",
                                                  "QC In Progress", "Completed", "Rework", "Hold"], key="bd_status")

        bd_sql = """
            SELECT f.id, f.filename AS Filename, c.name AS Client, f.batch AS Batch,
                   f.pages AS Pages, f.status AS Status
            FROM files f LEFT JOIN clients c ON f.client_id=c.id
            WHERE 1=1
        """
        bd_params = []
        if bd_client != "All":
            bd_sql += " AND c.name=?"; bd_params.append(bd_client)
        if bd_batch != "All":
            bd_sql += " AND f.batch=?"; bd_params.append(bd_batch)
        if bd_status != "All":
            bd_sql += " AND f.status=?"; bd_params.append(bd_status)
        bd_sql += " ORDER BY f.batch, f.filename"

        bd_df = df_query(bd_sql, tuple(bd_params))

        if bd_df.empty:
            st.info("No files match the filter above.")
        else:
            st.warning(f"⚠️ **{len(bd_df)} file(s)** will be deleted. Preview:")
            st.dataframe(bd_df.drop(columns=["id"]), use_container_width=True, hide_index=True, height=280)

            total_pages = int(bd_df["Pages"].sum())
            st.markdown(f"**Total pages that will be deleted: {total_pages}**")

            confirm_col1, confirm_col2 = st.columns([3, 1])
            with confirm_col1:
                bd_confirm = st.checkbox(
                    f"✓ I confirm — permanently delete these {len(bd_df)} file(s)",
                    key="bd_confirm"
                )
            with confirm_col2:
                bd_btn = st.button(
                    "🗑️ Delete All",
                    type="primary",
                    disabled=not bd_confirm,
                    use_container_width=True,
                    key="bd_execute"
                )

            if bd_btn and bd_confirm:
                ids = bd_df["id"].tolist()
                id_placeholders = ",".join("?" * len(ids))
                with st.spinner(f"Deleting {len(ids)} files..."):
                    conn = get_conn()
                    conn.execute(f"DELETE FROM qc_checks WHERE file_id IN ({id_placeholders})", ids)
                    conn.execute(f"DELETE FROM files WHERE id IN ({id_placeholders})", ids)
                    conn.commit()
                    conn.close()
                st.success(f"✅ Deleted {len(ids)} file(s) successfully.")
                st.toast(f"🗑️ {len(ids)} files deleted", icon="✅")
                st.rerun()
...
def page_files_trainee():
    ...
    sql = """SELECT f.id, f.filename, c.name AS client_name, f.batch, f.pages,
                    ...
                    f.error_type, f.qc_remark, f.notes
             FROM files f
             LEFT JOIN clients c ON f.client_id = c.id
    ...
    for _, f in files.iterrows():
        ...
        with st.expander(title, expanded=expanded):
            ...
            with col1:
                ...
                qc_remark = safe(f.get("qc_remark", ""), "")
                if f["rework"] == "Yes" and err_type and err_type not in ("None", "-"):
                    st.error(f"🔴 **QC failed: {err_type}**")
                if f["rework"] == "Yes" and qc_remark:
                    st.error(f"💬 **QC team comment:**\n\n{qc_remark}")
                    st.caption("Read this carefully — fix exactly what the QC team described, then click Start Rework.")
            ...
            if f["status"] in ("Pending", "Hold"):
                ...
                qc_remark_val = safe(f.get("qc_remark", ""), "")
                if qc_remark_val:
                    st.error(f"🔁 **QC failed. What to fix:**\n\n> _{qc_remark_val}_\n\nFix this exact issue → upload corrected file to Drive 02_Completed → click Start Rework.")
                else:
...
def _render_qc_file_card(f, current_user_id, mode):
    ...
    with st.expander(f"🔍 **{f['filename']}** — {f['client']} — by {f['producer']} — {f['pages']} pages{rework_label}",
        ...
        with st.form(f"qc_{f['id']}"):
            ...
            opts = ["Pass", "Fail", "Other", "N/A"]
            qc1, qc2, qc3 = st.columns(3)
            with qc1:
                heading = st.selectbox("Heading (H1/H2)", opts, index=opts.index(defaults["heading"]) if defaults["heading"] in opts else 0)
                tables = st.selectbox("Tables/Captions", opts, index=opts.index(defaults["tables"]) if defaults["tables"] in opts else 0)
            with qc2:
                figure = st.selectbox("Figure/Alt-Text", opts, index=opts.index(defaults["figure"]) if defaults["figure"] in opts else 0)
                links = st.selectbox("Links/Footnotes", opts, index=opts.index(defaults["links"]) if defaults["links"] in opts else 0)
            with qc3:
                reading_order = st.selectbox("Reading Order", opts, index=opts.index(defaults["reading_order"]) if defaults["reading_order"] in opts else 0)
                pac = st.selectbox("PAC Report", opts, index=opts.index(defaults["pac"]) if defaults["pac"] in opts else 0)
            ...
            st.caption("⚠️ This remark will be visible to the producer so they know exactly what to fix.")

            checks = [heading, figure, reading_order, tables, links, pac]
            fail_count = sum(1 for x in checks if x == "Fail")
            other_count = sum(1 for x in checks if x == "Other")
            if fail_count > 0:
                st.error(f"❌ Overall: FAIL ({fail_count} criteria failed)")
            elif other_count > 0:
                st.warning(f"⚠️ Overall: NEEDS REVIEW ({other_count} criteria marked 'Other')")
            else:
            ...
            with btn2:
                if st.form_submit_button("✅ Pass — Ready for client", type="primary", use_container_width=True):
                    with st.spinner("Marking as passed..."):
                        ...
                        execute("UPDATE files SET status='Completed', rework='No', submission_date=?, qc_remark=? WHERE id=?",
                                (date.today().isoformat(), remark, int(f["id"])))
            ...
            with btn3:
                if st.form_submit_button("❌ Fail — Back to producer", use_container_width=True):
                    if not remark.strip():
                        st.error("Please describe the issue in the Remark box before failing — the producer needs to know what to fix.")
                    else:
                        with st.spinner("Sending back for rework..."):
                            ...
                            first_fail = next(((label, val) for (label, val) in error_map if val in ("Fail", "Other")), ("Other", "Fail"))
                            execute("""UPDATE files SET status='Rework', rework='Yes',
                                       errors_count=COALESCE(errors_count,0)+?, error_type=?,
                                       qc_remark=?, qc_picked_by_id=NULL WHERE id=?""",
                                    (fail_count + other_count, first_fail[0], remark, int(f["id"])))
